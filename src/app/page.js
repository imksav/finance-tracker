  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      transData.forEach((t) => {
        ...
        // FIXED: Removed the extra space in "Wife"
        if (type === "Expense" || type === "Wife") {
          const c2 = t.category2?.name;
          if (c2) catExpenseMap[c2] = (catExpenseMap[c2] || 0) + val;
        }
        ...
        if (profileDate) {
          ...
          if (entryTime > updateTime) {
            ...
            // ADDED: Subtract "Wife" from live balance
            if (type === "Wife") tempBalanceChange -= val;
          }
        }
      });
